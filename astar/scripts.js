        ...
        while (openset.length > 0) {
            ...
            for (i = 0; i < neighbors.length; i++) {
                if (this.isNodeInList(neighbors[i].node, closedset)) {
                    if (debug) {
                        console.log("Neighbor is already visited");
                    }
                    continue;
                }
                ...
                if (!this.isNodeInList(neighbors[i].node, openset) || tentative_g_score < neighbors[i].g) { // If neighbor is not in openset
                    ...
                    if (!this.isNodeInList(neighbors[i].node, openset)) {
                        if (debug) {
                            console.log(neighbors[i] + ' pushed to openset'); 
                        }
            ...
            if (debug) {
              console.log("Node: (" + current.node[0] + ", " + current.node[1] + ") with G: " + current.g + " - F: " + current.f);
            }
            if (count++ >= 100)
                break;
        } // End while
